...
import {
  mockCreatePatronApiServerError,
  mockCreatePatronApiOffline,
  mockUsernameApi,
} from "../../utils/mock-api";

const lang = "en";

test.describe("review page in English (en)", () => {
  ...
  test.describe("displays elements on review page", () => {
    ...
    test("displays Address section", async () => {
    ...
    test("displays Account section", async () => {
  ...
  test.describe("edits patron information on review page", () => {
    test("displays editable Personal information section", async () => {
    ...
    test("displays editable Account section", async () => {
  ...
  test.describe("mocks API responses on review page", () => {
    ...
    test("displays server error message when create-patron API returns a 502", async ({
      page,
    }) => {
      await mockCreatePatronApiServerError(page);
      await reviewPage.submitButton.click();
      await expect(reviewPage.serverErrorMessage).toBeVisible();
    });

    test("displays server error message when user is offline", async ({
      page,
    }) => {
      await mockCreatePatronApiOffline(page);
      await reviewPage.submitButton.click();
      await expect(reviewPage.serverErrorMessage).toBeVisible();
    });
  });
