...
for (const { lang, name } of SUPPORTED_LANGUAGES) {
  ...
  const TEST_EDITED_ACCOUNT = createTestAccount();
  test.describe.serial(
    `E2E: Edits patron information in ${name} (${lang})`,
    { tag: "@e2e" },
    () => {
      ...
      test("displays error when address is too long", async ({ page }) => {
        ...
        await test.step("enters invalid home address", async () => {
          ...
          await expect(pageManager.addressPage.nextButton).toBeVisible();
          await pageManager.addressPage.nextButton.click();
          await waitForErrorOrHeading(
            getErrorMessages(pageManager.addressPage),
      ...
      test("retains user-entered info after clicking breadcrumb", async ({
        ...
        await test.step("enters personal information", async () => {
          ...
          await clickCheckbox(
            pageManager.personalPage.receiveInfoCheckboxLabel,
        ...
        await test.step("retains info on account page", async () => {
          ...
          await expect(
            pageManager.accountPage.acceptTermsCheckbox
          ).toBeChecked();
        });
      });
    }
