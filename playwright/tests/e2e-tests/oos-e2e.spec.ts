...
for (const { lang, name } of SUPPORTED_LANGUAGES) {
  test.describe.serial(
    `E2E: Complete OOS patron application with Sierra API integration in ${name} (${lang})`,
    { tag: ["@release", "@e2e"] },
    () => {
      ...
      test("submits OOS patron application", async ({ page }) => {
        ...
        await test.step("verifies home and alternate addresses", async () => {
          ...
          await pageManager.addressVerificationPage
            ...
            .click();
          await pageManager.addressVerificationPage
            .getAlternateAddressOption(TEST_OOS_ADDRESS_2ND.street)
            .click();
          await clickNextButton(
        ...
        await test.step("displays home and alternate addresses on review page", async () => {
          ...
          await expect(
            pageManager.reviewPage.getText(TEST_OOS_ADDRESS_2ND.street)
          ).toBeVisible();
          await expect(
            pageManager.reviewPage.getText(TEST_OOS_ADDRESS_2ND.city)
          ).toBeVisible();
          await expect(
            pageManager.reviewPage.getText(TEST_OOS_ADDRESS_2ND.state)
          ).toBeVisible();
          await expect(
            pageManager.reviewPage.getText(TEST_OOS_ADDRESS_2ND.postalCode)
          ).toBeVisible();
        });
