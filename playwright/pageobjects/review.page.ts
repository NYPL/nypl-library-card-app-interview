...
export class ReviewPage {
  ...
  constructor(page: Page, appContent?: any, lang?: string) {
    ...
    this.passwordError = page.getByText(
      cleanText(appContent?.account?.errorMessage?.password, {
        textFilter: function (text) {
          return text.replace(/&amp;/g, "&");
        },
      }) ||
        "There was a problem. Your password must be between 8 and 32 characters using upper or lower case characters (a-z, A-Z), numbers (0-9), and/or special characters limited to the following: . ~ ! ? @ # $ % ^ &amp; * ( )"
    );
    ...
    this.serverErrorMessage = page.getByText(
